...
class DataGenerator:
        ...
        while current_time < self.stop_timestamp:
            ...
            for sensor_ship in self.sensor_ship_list:
                ...
                if self.clutter is True:  # 如果开启环境杂波
                    clutter_num = np.random.randint(5, 15)  # 在这一帧的杂波点数量
                    ...
                    for i in range(clutter_num):  # 生成雷达杂波数据
                        around_ship_radar_data.append({
                            ...
                            "NO": NO_list[i],
                            "SendTime": current_time,
                            "Lon": sensor_ship['Lon'] + np.random.uniform(-0.1, 0.1),
                            "Lat": sensor_ship['Lat'] + np.random.uniform(-0.1, 0.1),
    ...
    def geo_to_csv(sensory_data, around_ais_data, around_radar_data):
        ...
        around_radar_data.to_csv('simulation_ship_target.csv', index=False)

    def read_geo_csv(self):
        sensory_data = pd.read_csv('simulation_ship_sensor.csv')
        # 将 WKT 字符串转换为几何对象
        sensory_data['geometry'] = sensory_data['geometry'].apply(wkt.loads)
